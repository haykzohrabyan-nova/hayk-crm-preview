...
type Stage = "New" | "Claimed" | "Contacted" | "Routed to Sales" | "Quote Sent" | "Won" | "Rejected";
...
const LEADS: Lead[] = [
  {
    ...
    quotes: [
      { ref: "QO-2026-0189", amount: 18450, sentDaysAgo: 2, status: "Sent" },
      { ref: "QO-2026-0172", amount: 21300, sentDaysAgo: 5, status: "Viewed" },
    ],
    previousOrdersList: [
      { ref: "ORD-2026-0091", amount: 14250, shipped: "Jun 10, 2026" },
      { ref: "ORD-2026-0067", amount: 8760, shipped: "May 2, 2026" },
      { ref: "ORD-2026-0032", amount: 5430, shipped: "Mar 18, 2026" },
    ],
    ...
    commHistory: [
      ...
      { id: "c4", type: "email_out", at: "Jul 1 · 11:11 AM", atRel: "7m ago", author: "Maria Hakobyan", subject: "Quote for folding cartons + labels · Grimeylyfe Records", body: "Hi Grim, thanks for the call. Attached is the quote for 5,000 folding cartons and 5,000 labels with rush production for July 15 delivery. Let me know if you have questions.", attachments: [{ name: "QO-2026-0189.pdf", size: "246 KB", kind: "pdf" }, { name: "Bazaar-Capabilities.pdf", size: "1.2 MB", kind: "pdf" }] },
...
export default function LeadsPreview() {
  ...
  const sourceOptions = useMemo(() => {
    ...
    PADDED_LEADS.forEach(l => s.add(l.source));
    return Array.from(s).sort();
  }, []);
  const sdrOptions = useMemo(() => {
    const s = new Set<string>();
    PADDED_LEADS.forEach(l => s.add(l.sdrOwner));
    return Array.from(s).sort();
  }, []);
  ...
  const filtered = useMemo(() => {
    ...
    const combined = [...userLeads, ...PADDED_LEADS];
  ...
  const counts: Record<TabKey, number> = useMemo(() => {
    const c: any = {};
    TAB_DEFS.forEach(t => (c[t.key] = PADDED_LEADS.filter(t.predicate).length));
    return c;
  }, []);
  ...
  return (
    <div style={{ fontFamily: "system-ui, -apple-system, sans-serif", background: "var(--preview-bg)", color: "var(--preview-text)", borderRadius: "14px", padding: "20px", margin: "-20px" }}>
      ...
      <div style={{ display: "flex", gap: "8px", marginBottom: "18px", alignItems: "center", padding: "10px 14px", background: "var(--preview-surface-2)", borderRadius: "10px", color: "var(--preview-text)", border: "1px solid var(--preview-border)" }}>
        ...
        <span style={{ fontSize: "12px", color: "var(--preview-text)" }}>Leads · fully interactive · click any row to open the side panel</span>
  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "18px", gap: "16px" }}>
        <div>
          <h1 style={{ fontSize: "22px", fontWeight: 800, color: "var(--preview-text)" }}>Leads</h1>
          <div style={{ fontSize: "12px", color: "var(--preview-text-muted)", marginTop: "2px" }}>Manage, qualify, and route new leads</div>
        </div>
        <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
          ...
          <button onClick={onOpenAdd} style={{ background: ACCENT, color: "#fff", border: "none", padding: "9px 16px", borderRadius: "10px", fontWeight: 700, fontSize: "13px", cursor: "pointer" }}>+ Add Lead</button>
        </div>
      </div>
