...
import {
  commitBulkLeadImport,
  parseBulkLeadImportFile,
  validateBulkLeadImport,
} from "@/lib/utils/bulk-import-leads";

export const runtime = "nodejs";

// POST /api/admin/leads/import?dry_run=true — validate only (no DB writes for leads)
// POST /api/admin/leads/import — import valid rows
...
export async function POST(request: NextRequest) {
  ...
  const parsed = parseBulkLeadImportFile(raw);
  if (!parsed.ok) {
  ...
  try {
    if (dryRun) {
      const summary = await validateBulkLeadImport(admin, parsed.file);
      return NextResponse.json({ dry_run: true, ...summary });
    }

    const summary = await commitBulkLeadImport(admin, parsed.file, userId!);
    return NextResponse.json({ dry_run: false, ...summary });
  } catch (err) {
    console.error("[admin/leads/import]", err);
