...
import {
  commitBulkCustomerImport,
  parseBulkCustomerImportFile,
  validateBulkCustomerImport,
} from "@/lib/utils/bulk-import-customers";

export const runtime = "nodejs";

// POST /api/admin/customers/import?dry_run=true — validate only (no DB writes)
// POST /api/admin/customers/import — import valid rows
...
export async function POST(request: NextRequest) {
  ...
  const parsed = parseBulkCustomerImportFile(raw);
  if (!parsed.ok) {
  ...
  try {
    if (dryRun) {
      const summary = await validateBulkCustomerImport(admin, parsed.file);
      return NextResponse.json({ dry_run: true, ...summary });
    }

    const summary = await commitBulkCustomerImport(admin, parsed.file, userId!);
    return NextResponse.json({ dry_run: false, ...summary });
  } catch (err) {
    console.error("[admin/customers/import]", err);
