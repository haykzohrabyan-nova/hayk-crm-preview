...
**Open vs Claim:** Unclaimed → `POST /claim` (sets `sales_status = Claimed`). Owned lead → modal without `POST /lock` when `sales_owner_id = you`. Rep clicks **In Progress** in modal (from Claimed) → `POST /in-progress` with `{ role: "sales" }`.
