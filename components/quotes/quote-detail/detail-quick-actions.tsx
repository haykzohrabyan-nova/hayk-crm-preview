...
import { resolveOutreachChannelKind } from "@/lib/utils/outreach-channel-display";
...
import { ResendQuoteModal, type SendChannelOpts } from "@/components/quotes/quote-detail/resend-quote-modal";
...
export function DetailQuickActions({
  ...
  /** Current send channel from paymentDraft / ticket — prefills the send modal. */
  sendChannel,
  /** Current send email destination — prefills the send modal. */
  sendEmail = "",
  /** Current send phone destination — prefills the send modal. */
  sendPhone = "",
}: {
  ...
  onSendQuote?: (opts?: SendChannelOpts) => void;
  ...
  const [resendModal, setResendModal] = useState<"quote" | "invoice" | null>(null);
  const [invoiceSendErr, setInvoiceSendErr] = useState<string | null>(null);
  const [invoiceSendMsg, setInvoiceSendMsg] = useState<string | null>(null);
  ...
  const showStaffCancel   = !!onCancelTicket && canStaffCancelTicket(userRole, ticket);
  const showRecordRefund  = !!onRecordRefund && canRecordRefund(ticket, priorRefunds, userRole);
  ...
  async function handleSendViaModal(opts: SendChannelOpts) {
    if (resendModal === "quote") {
      onSendQuote?.(opts);
      // onSendQuote is synchronous (triggers handleSave in parent); modal closes immediately
    } else {
      // "invoice" — PATCH with invoice_channel + invoice_destination
      setInvoiceSendMsg(null);
      setInvoiceSendErr(null);

      const invoiceDestination =
        opts.channel === "email" ? opts.email
        : opts.channel === "sms"  ? opts.phone
        : opts.phone; // "both" — API uses SMS as primary destination; email is stored separately
      ...
      const res = await fetch(`/api/tickets/${ticket.id}`, {
        ...
        body: JSON.stringify({
          resend_invoice: true,
          invoice_channel: opts.channel,
          invoice_destination: invoiceDestination,
        }),
      });
      const data = await res.json();
      if (!res.ok) {
        throw new Error(data.error ?? "Failed to send invoice link.");
      }
      setInvoiceSendMsg(`Sent via ${data.channel ?? opts.channel}.`);
  ...
  return (
    <>
      <div className="mt-3 md:mt-4 flex flex-col gap-2">
        ...
        {(invoiceSendMsg || invoiceSendErr) && (
          <p
            className="text-xs px-3 py-2 rounded-lg border"
            style={{
              borderColor: invoiceSendErr ? "var(--color-danger-border)" : "var(--color-success-border)",
              background:  invoiceSendErr ? "var(--color-danger-bg)"    : "var(--color-success-bg)",
              color:       invoiceSendErr ? "var(--color-danger)"        : "var(--color-success)",
            }}
          >
            {invoiceSendErr ?? invoiceSendMsg}
          </p>
        )}

        {primaryActionCount > 0 && (
          <div className={`grid gap-2 ${primaryActionCount > 1 ? "grid-cols-2" : "grid-cols-1"} md:grid-cols-1`}>
            {canMarkComplete && (
              <button
                type="button"
                disabled={saving}
            ...
            {canResendInvoice && (
              <button
                type="button"
                disabled={saving}
                onClick={() => setResendModal("invoice")}
                title="Resend invoice link to customer"
                className={`${btnBase} w-full border hover:opacity-80`}
                style={{ borderColor: "var(--color-border)", color: "var(--color-text-primary)", background: "var(--color-surface)" }}
              >
        ...
        {showQuoteLink && (
          <div className="grid grid-cols-2 gap-2 md:grid-cols-1">
            ...
            <button
              ...
              style={{
                borderColor:
                  copyState === "copied" ? "var(--color-success-border)"
                  : copyState === "error" ? "var(--color-danger-border)"
                  : "var(--color-border)",
                color:
                  copyState === "copied" ? "var(--color-success)"
                  : copyState === "error" ? "var(--color-danger)"
                  : "var(--color-text-muted)",
                background:
                  copyState === "copied" ? "var(--color-success-bg)"
                  : copyState === "error" ? "var(--color-danger-bg)"
                  : "var(--color-surface)",
              }}
            >
      ...
      <ResendQuoteModal
        isOpen={resendModal !== null}
        mode={resendModal ?? "quote"}
