  ...
  async function downloadSample() {
    ...
    try {
      ...
      if (!res.ok || !data.template) {
        ...
        setError(msg);
        reportApiError(msg, res, "LeadsImport/sample");
        setLoading(null);
        return;
      }
      ...
      setError(msg);
      reportApiError(msg, { status: 0, url: "/api/admin/leads/import/template" }, "LeadsImport/sample", { originalError: String(err) });
    }
    setLoading(null);
  }
  ...
  async function runValidate() {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        setError(msg);
        reportApiError(msg, res, "LeadsImport/validate");
        setLoading(null);
        return;
      }
      setSummary(data);
      ...
      setError(msg);
      reportApiError(msg, { status: 0, url: "/api/admin/leads/import" }, "LeadsImport/validate", { originalError: String(err) });
    }
    setLoading(null);
  }
    ...
    try {
      for (let i = 0; i < total; i += IMPORT_CHUNK_SIZE) {
        ...
        if (!res.ok) {
          ...
          setError(msg);
          reportApiError(msg, res, "LeadsImport/import");
          setLoading(null);
          setProgress(null);
          return;
        }
      ...
      const msg = "Network error — please try again.";
      setError(msg);
      reportApiError(msg, { status: 0, url: "/api/admin/leads/import" }, "LeadsImport/import", { originalError: String(err) });
    }
    setLoading(null);
    setProgress(null);
  }
