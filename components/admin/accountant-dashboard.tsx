...
export function AccountantDashboard() {
  ...
  const privacy = useDashboardPrivacy(() => fetchCountsRef.current());
  ...
  const fetchCounts = useCallback(async () => {
    setLoading(true);
    const d = await fetch("/api/payments/counts").then((r) => r.json());
    if (typeof d.values_hidden === "boolean") {
      privacy.syncFromApi(d.values_hidden);
    }
  ...
  const countValues = [
    counts?.pending_evidence ?? 0,
    counts?.orders_in_production ?? 0,
    counts?.completed_this_month ?? 0,
  ];
  ...
  return (
    <div className="space-y-6">
          ...
          >
            <div className="flex items-center justify-between mb-3">
              ...
              <span
                ...
                style={{
                  background: card.accent ? "var(--color-badge-bg)" : "var(--color-row-alt)",
                  color: card.accent ? "var(--color-accent)" : "var(--color-text-muted)",
                }}
              >
                {card.icon}
              </span>
            ...
            <div
              ...
              style={{ color: card.accent ? "var(--color-accent)" : "var(--color-text-primary)" }}
            >
              {loading ? (
                "—"
              ) : metricsHidden ? (
                <DashboardHiddenValue size="lg" kind="count" />
              ) : (
                countValues[i]
              )}
            </div>
      ...
      {!metricsHidden && counts && counts.pending_evidence > 0 && (
        <div
